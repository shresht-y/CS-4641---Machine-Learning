...
class RandomForest(object):
    ...
    def _bootstrapping(self, num_training, num_features, random_seed = None):
        ...
        row_idx = np.random.choice(num_training,num_training)# TODO
        #################################################################################

        ############# Get Col Indices Second - write your code below ####################
        size = int(num_features*self.max_features)
        col_idx = np.random.choice(num_features,size,replace=False)# TODO
        ##################################################################################
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_estimators):
            #select a set of rows, and pull the columns associated with it up to i 
            newX = X[self.bootstraps_row_indices[i]][:, self.feature_indices[i]]
            newY = y[self.bootstraps_row_indices[i]]
