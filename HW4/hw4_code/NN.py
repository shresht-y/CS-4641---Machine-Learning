...
class NeuralNet:
    ...
    def derivative_leaky_relu(self, alpha, u):
        ...
        """
        # TODO
        lrelu = self.leaky_relu(alpha, u)
        return np.where(lrelu>0,1,alpha)
    ...
    def softmax(self, u):
        ...
        """
        u = u - np.max(u,1,keepdims=True) 
        esums = np.exp(u)
        u = np.exp(u)
        esums = np.sum(esums, axis=1,keepdims=True)
        return np.divide(u,esums)
    ...
    def _dropout(u, prob):
        ...
        if prob == 0.0:
            ...
            return np.zeros((len(u),len(u[0])))
        
        mask = np.random.choice([0,1],np.shape(u),p=[prob, 1-prob])
        after_dropout = u * mask
        after_dropout = after_dropout / (1 - prob)
        return after_dropout, mask
    ...
    def cross_entropy_loss(self, y, y_hat):
        ...
        """
        # TODO
        sum_term = y * np.log(y_hat)
        sum_term = np.sum(sum_term)
        return -1 * (sum_term/np.shape(y)[0])
    ...
    def forward(self, x, use_dropout):
        ...
        self.cache["X"] = x
        # x*self.parameters["theta1"]
        u1 = np.matmul(x, self.parameters["theta1"]) + self.parameters["b1"]  # TODO
        o1 = self.leaky_relu(self.alpha,u1) #TODO
        ...
        # self.parameters["theta2"]*o1
        u2 = np.matmul(o1, self.parameters["theta2"]) + self.parameters["b2"]# TODO
        o2 = self.softmax(u2)  # TODO
    ...
    def compute_gradients(self, y, yh, use_dropout):
        ...
        n = np.shape(y)[0]
        dLoss_u2 = (yh-y)/n
        dLoss_theta2 = np.matmul(np.transpose(self.cache["o1"]),dLoss_u2)
        dLoss_b2 = np.sum(dLoss_u2,axis=0)
        dLoss_o1 = np.matmul(dLoss_u2, np.transpose(self.parameters["theta2"]))

        if use_dropout:
            dLoss_u1 = dLoss_o1 * self.derivative_leaky_relu(self.alpha,self.cache["u1"]) * self.cache["mask"] * (1/(1-self.dropout_prob))
        else:
            dLoss_u1 = dLoss_o1 * self.derivative_leaky_relu(self.alpha,self.cache["u1"])
        

        dLoss_theta1 = np.transpose(np.matmul(np.transpose(dLoss_u1),self.cache["X"]))
    ...
    def gradient_descent(self, x, y, iter=60000, use_momentum=False, local_test=False):
        ...
        **For LOCAL TEST append and print out loss at every iteration instead of every 1000th multiple.
        ...
        for i in range(iter):
            ...
            output = self.forward(x, self.use_dropout)
            self.backward(y, output,self.use_dropout,use_momentum)
    ...
    def batch_gradient_descent(self, x, y, use_momentum, iter=60000, local_test=True):
        """
